"""
scenario.py — Synthetic account scenario generator for uploaded PDFs.

Given a filename and optional LLM-extracted contract fields, deterministically
generates a complete account scenario (account, contract, usage, invoice) that
the existing 4-agent pipeline can analyze.

Seed is derived from filename MD5 so the same file always produces the same data.
"""
...
# Three tiers; seed % 3 selects the tier
_TIERS = [
    {
        "name": "Professional",
        "base_fee": 15000,
...
def _get_tier(seed: int) -> dict:
    ...
    # ±15% variance on base_fee only
    variance = 1.0 + rng.uniform(-0.15, 0.15)
    tier["base_fee"] = round(tier["base_fee"] * variance / 100) * 100  # round to $100
    return tier
...
def _make_contract(account_id: str, contract_id: str, tier: dict, extracted: dict) -> dict:
    ...
    ovs = tier["overage_section"]
    lms = tier["limit_section"]
    dcs = tier["discount_section"]
...
def _make_usage(account_id: str, contract: dict, seed: int) -> dict:
    ...
    # Distribute across 31 days so sum == total_units
    daily_units = []
...
def _make_invoice(account_id: str, contract_id: str, contract: dict, usage: dict, seed: int) -> tuple[dict, str]:
    ...
    if error_type == "missing_overage":
        # Base fee (discounted correctly) but overage line is $0
        line_items = [
            {
                "id": "li-001",
        ...
        # Discount applied to overage too (should be base only)
        ...
        line_items = [
            ...
            },
            {
                ...
                "discount_rate": discount_rate,  # WRONG per contract
        ...
        # Overage charged at wrong (lower) rate
...
def build_full_scenario(filename: str, extracted: dict) -> dict:
    """
    Build and save a complete account scenario from a filename + optional
    LLM-extracted fields.  Returns the account dict.

    Always succeeds — missing extracted fields fall back to synthetic defaults.
    """
    seed = _seed_from_filename(filename)
    ...
    invoice, _ = _make_invoice(account_id, contract_id, contract, usage, seed)

    # ARR from final base_fee
